...
// - 과제1. Person 타입, key 값이 주어졌을 때 Value를 불러오는 함수를 만들어 봐. Key는 아무거나 될 수 있음. 함수명 : getProperty
function getProperty(person: Person, key: keyof Person) {
    return person[key];
}

// - 과제2. 특정 인스턴스가 주어졌을 때, 해당 인스턴스의 Key에 대한 값을 불러오는 함수를 구하려면?
function getPropertyFromInstance(instance: Person, key: keyof typeof instance) {
    return instance[key];
}